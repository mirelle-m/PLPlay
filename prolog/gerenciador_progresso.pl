:- module(gerenciador_progresso, [
    ...
    marcar_acerto/1,
    pergunta_missao/6,
    contar_perguntas_faltantes/3,
    verificar_missao_completa/2
]).
...
contar_perguntas_faltantes(UsuarioID, MissaoID, Contagem) :-
    carregar_missao(UsuarioID, MissaoID),
    findall(1, pergunta_missao(_, _, _, _, _, nao_acertada), Faltantes),
    length(Faltantes, Contagem).

verificar_missao_completa(UsuarioID, MissaoID) :-
    carregar_missao(UsuarioID, MissaoID),
    findall(1, pergunta_missao(_, _, _, _, _, acertada), Acertadas),
    length(Acertadas, Contagem),
    Contagem >= 4.