...
salvarRespostaAcertada pergunta = do
  -- ver se isso funfa!
    appendFile "../data/acertos.csv" (perguntaParaCSV pergunta ++ "\n")
...
exibirResumo resultado = do
    ...
    if numErros >= maxErrosPermitidos nivelAtual
        then do
            ...
            putStrLn "            GAME OVER!            "
            putStrLn "===================================================================="
            -- putStrLn ("Você excedeu o limite de erros para este nível " ++ show nivelAtual)
            putStrLn ("Você excedeu o limite de erros para este nível :(")
    ...
    if not (null (acertos resultado))
        then do
            ...
            putStrLn ""
        else putStrLn "Poxa! Não foi dessa vez...\n"
...
perguntaMissao::Int->IO Int
perguntaMissao missaoAtual = do
  putStrLn "Qual missão você quer jogar?"
  input <- getLine
  let missaoDesejada = read input :: Int
  if missaoDesejada > missaoAtual then do
    putStrLn "Ops! Missão não desbloqueada... Tente novamente!"
    perguntaMissao missaoAtual
  else return missaoDesejada
...
main = do
  ...
  putStrLn "Escolha o nível de dificuldade para essa missão:"
  ...
  case stringParaNivel nivelInput of
    Nothing -> do
      putStrLn "Nível inválido! Usando nivel Medio por padrao."
      main
    Just nivelEscolhido -> do
      putStrLn ("Nivel escolhido: " ++ show nivelEscolhido)
      
      -- ver como pegar essa informação da missão máxima alcançada
      missaoDesejada <- perguntaMissao 9

      putStrLn ("Quantas perguntas da missão '" ++ show missaoDesejada ++ "' você quer responder?")
      numInput <- getLine
      ...
      let perguntasDaMissao = filter (\p -> missao p == show missaoDesejada) todasAsPerguntas
      
      let perguntasParaExibir = take numPerguntas perguntasDaMissao
      
      if null perguntasParaExibir
        then putStrLn "Nenhuma pergunta encontrada para esta missão ou quantidade."
        else do
          putStrLn ("\n--- INICIANDO QUIZ COM " ++ show (length perguntasParaExibir) ++ " PERGUNTAS DA MISSAO '" ++ show missaoDesejada ++ "' ---")
